...
function InternalLogoutButton({
	...
	async function handleLogout() {
		try {
			const response = await fetch("/api/internal/logout", {
				method: "POST",
				headers: {
			...
			window.dispatchEvent(new CustomEvent(INTERNAL_DASHBOARD_LOGOUT_EVENT));
			window.location.assign(payload.redirectTo ?? "/internal/login");
		} catch {
...
export function Header() {
	...
	const showDashboardLogout =
		pathname.startsWith("/internal") && pathname !== "/internal/login";
		...
		<header className="sticky top-0 z-50 will-change-transform">
				...
				<div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
					<div className="flex items-center justify-end gap-4">
						...
						{showDashboardLogout && (
							<>
								<div className="h-4 w-px bg-border" />
								<InternalLogoutButton className="text-sm font-medium text-muted-foreground transition-colors hover:text-foreground" />
							</>
						)}
					</div>
				</div>
			...
			<div className="bg-background shadow transform-gpu">
				<nav className="mx-auto flex max-w-7xl px-4 sm:px-6 lg:px-8 items-center justify-between py-2 sm:py-4 md:py-6">
					...
					>
						<span>
							Rubadue <span className="text-primary ">Wire</span>
						</span>
						<svg
					...
					<div className="hidden lg:flex lg:flex-1 lg:justify-end lg:items-center lg:gap-1">
						<DropdownMenu>
							...
							<DropdownMenuContent
								align="start"
								className="w-[500px] lg:w-[650px] p-0"
							>
										...
										>
											...
											<div className="relative z-10">
												...
												<p className="text-xs leading-tight text-primary-foreground/90">
													Explore our complete range of high-performance wire
													products.
												</p>
											</div>
										</a>
								...
								<div className="flex h-full w-full flex-col">
									...
									<SheetFooter className="flex-none border-t p-6">
										<div className="flex w-full flex-col gap-3">
											...
											{showDashboardLogout && (
												<InternalLogoutButton className="flex w-full items-center justify-center rounded-md border border-input bg-background px-3 py-2 text-sm font-semibold text-foreground shadow-sm hover:bg-accent hover:text-accent-foreground" />
											)}
										</div>
									</SheetFooter>
								</div>
